...
[`berkeley`](berkeley.html) for the full five-department version of the
Berkeley admissions data. Note that `berk_sub` uses `male` (1 = male)
while `berkeley` uses `female` (1 = female).
